                ...
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:620px;background:#ffffff;border:1px solid #e8e8e8;border-radius:18px;overflow:hidden;">
                    <tr>
                        <td align="center" style="padding:28px 24px 22px;background:#fff7f7;">
                            ...
                            <p style="margin:22px 0 6px;color:#d71920;font-size:13px;font-weight:700;letter-spacing:1.2px;text-transform:uppercase;">Order cancelled</p>
                            <h1 style="margin:0;color:#1f1f1f;font-size:28px;line-height:1.25;">Your order has been cancelled</h1>
                            <p style="margin:12px 0 0;color:#606060;font-size:15px;line-height:1.6;">
                                Hi {{ $order->full_name }}, order <strong>{{ $order->number }}</strong> is now cancelled.
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding:24px;">
                            @if ($order->cancellation_reason)
                                <p style="margin:0 0 16px;color:#555;font-size:14px;line-height:1.6;">
                                    Reason: <strong style="color:#222;">{{ $order->cancellation_reason }}</strong>
                                </p>
                            @endif

                            @if ($order->payment_status === 'refund_pending')
                                <p style="margin:0 0 16px;color:#555;font-size:14px;line-height:1.6;">
                                    Your prepaid payment is marked for refund. Refunds are returned through the original payment method once processed.
                                </p>
                            @endif
                            ...
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
                                <tr>
                                    <td align="center" style="padding-top:8px;">
                                        <a href="{{ $orderUrl }}" style="display:inline-block;padding:14px 26px;background:#d71920;color:#ffffff;text-decoration:none;font-size:15px;font-weight:700;border-radius:999px;">View order</a>
                                    </td>
                                </tr>
                            </table>
                        </td>
